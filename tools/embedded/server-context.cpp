...
struct server_res_generator : server_core_res {
...
void server_routes::init_routes() {
    ...
    this->get_health = [this](const server_core_req &) {
    ...
    this->get_metrics = [this](const server_core_req & req) {
        auto res = create_response();
    ...
    this->get_slots = [this](const server_core_req & req) {
        auto res = create_response();
    ...
    this->post_slots = [this](const server_core_req & req) {
        auto res = create_response();
    ...
    this->get_props = [this](const server_core_req &) {
        auto res = create_response(true);
    ...
    this->post_props = [this](const server_core_req &) {
        auto res = create_response();
    ...
    this->get_api_show = [this](const server_core_req &) {
        auto res = create_response();
        std::string tmpl_default = common_chat_templates_source(meta->chat_params.tmpls.get(), "");
        json data = {
            {
    ...
    this->post_infill = [this](const server_core_req & req) {
        auto res = create_response();
    ...
    this->post_completions = [this](const server_core_req & req) {
        auto res = create_response();
        std::vector<raw_buffer> files; // dummy
    ...
    this->post_completions_oai = [this](const server_core_req & req) {
        auto res = create_response();
        std::vector<raw_buffer> files; // dummy
        const json body = json::parse(req.body);
        return handle_completions_impl(
    ...
    this->post_chat_completions = [this](const server_core_req & req) {
        auto res = create_response();
    ...
    this->post_responses_oai = [this](const server_core_req & req) {
        auto res = create_response();
        std::vector<raw_buffer> files;
	...
	this->post_anthropic_messages = [this](const server_core_req & req) {
        auto res = create_response();
        std::vector<raw_buffer> files;
    ...
    this->post_anthropic_count_tokens = [this](const server_core_req & req) {
        auto res = create_response();
        std::vector<raw_buffer> files;
        json body = convert_anthropic_to_oai(json::parse(req.body));
		SRV_DBG("%s\n", "Request converted: Anthropic -> OpenAI Chat Completions");
    ...
    this->post_apply_template = [this](const server_core_req & req) {
        auto res = create_response();
    ...
    this->get_models = [this](const server_core_req &) {
        auto res = create_response(true);
    ...
    this->post_tokenize = [this](const server_core_req & req) {
        auto res = create_response();
        const json body = json::parse(req.body);
    ...
    this->post_detokenize = [this](const server_core_req & req) {
        auto res = create_response();
        const json body = json::parse(req.body);
    ...
    this->post_embeddings = [this](const server_core_req & req) {
        return handle_embeddings_impl(req, TASK_RESPONSE_TYPE_NONE);
    };

    this->post_embeddings_oai = [this](const server_core_req & req) {
        return handle_embeddings_impl(req, TASK_RESPONSE_TYPE_OAI_EMBD);
    };

    this->post_rerank = [this](const server_core_req & req) {
        auto res = create_response();
    ...
    this->get_lora_adapters = [this](const server_core_req & req) {
        auto res = create_response();
    ...
    this->post_lora_adapters = [this](const server_core_req & req) {
        auto res = create_response();
        const json body = json::parse(req.body);
        if (!body.is_array()) {
...
std::unique_ptr<server_res_generator> server_routes::handle_slots_save(const server_core_req & req, int id_slot) {
    auto res = create_response();
...
std::unique_ptr<server_res_generator> server_routes::handle_slots_restore(const server_core_req & req, int id_slot) {
    auto res = create_response();
    const json request_data = json::parse(req.body);
    std::string filename = request_data.at("filename");
    if (!fs_validate_filename(filename)) {
...
std::unique_ptr<server_res_generator> server_routes::handle_slots_erase(const server_core_req & req, int id_slot) {
    auto res = create_response();
    auto & rd = res->rd;
    {
...
std::unique_ptr<server_res_generator> server_routes::handle_embeddings_impl(const server_core_req & req, task_response_type res_type) {
    auto res = create_response();
    if (!params.embedding) {
